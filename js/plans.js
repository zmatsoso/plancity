...
(function($) {
	...
	var premium_id = document.getElementById('href_services_value').value;
        new $.Zebra_Dialog({
           auto_focus_button: $('body.materialize').length ? false : true,
            source: {
                iframe: {
                    src: plancityData.root_url + '/benefits?id='+premium_id,
                    height: 500
                }
            },
            width: 800,
	...
	$('#href_other').on('click', function() {
	var premium_id = document.getElementById('href_other_value').value;
        new $.Zebra_Dialog({
           auto_focus_button: $('body.materialize').length ? false : true,
            source: {
                iframe: {
                    src: plancityData.root_url + '/other-info?id='+premium_id,
                    height: 500
                }
            },
            width: 800,
            title:  'Other Plan Info'
			
        });
    });
	
	$('#href_limits').on('click', function() {
	var premium_id = document.getElementById('href_limits_value').value;
        new $.Zebra_Dialog({
           auto_focus_button: $('body.materialize').length ? false : true,
            source: {
                iframe: {
                    src: plancityData.root_url + '/limits?id='+premium_id,
                    height: 500
                }
            },
            width: 800,
            title:  'Number and Age limits'
			
        });
    });

	$('#href_probation').on('click', function() {
	var premium_id = document.getElementById('href_probation_value').value;
        new $.Zebra_Dialog({
           auto_focus_button: $('body.materialize').length ? false : true,
            source: {
                iframe: {
                    src: plancityData.root_url + '/probations?id='+premium_id,
                    height: 500
                }
            },
            width: 800,
    ...
    $('#href_contacts').on('click', function() {
		var provider_id = document.getElementById('href_contacts_value').value;
		...
		new $.Zebra_Dialog({
            ...
            source: {
                iframe: {
                    ...
                    src: plancityData.root_url + '/contacts?id='+provider_id,
                    height: 500
                }
            },
            width: 800,
	...
	$('#supply_age').on('click', function() {
        new $.Zebra_Dialog('Please select your age and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
	...
	$('#supply_gender').on('click', function() {
        new $.Zebra_Dialog('Please select gender and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
	...
	$('#supply_category').on('click', function() {
        new $.Zebra_Dialog('Please Select Plan Category and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
	...
	$('#supply_plan_type').on('click', function() {
        new $.Zebra_Dialog('Please Select Plan Type and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
	...
	$('#supply_province').on('click', function() {
        new $.Zebra_Dialog('Please Select Province and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
	...
	$('#supply_area').on('click', function() {
        new $.Zebra_Dialog('Please Select Nearest Area to you and try again!', {
            auto_focus_button: $('body.materialize').length ? false : true,
            title: 'Error',
            type: 'error'
    });
    ...
    setTimeout(function() {
		...
		//$plans = $("#search_plans");
		$("#search_plans").on("click", search_now);



function search_now(){
var select_director = document.getElementById("select_director").value;
var select_age = document.getElementById("select_age").value;
var senior_citizen = document.getElementById("primary_checkbox").checked;
...
if(category=="Select Plan Category"){
supply_category();
}else{
...
if(select_province=="Select Your Province"){
supply_province();
}else{
	
var select_area = document.getElementById("select_area").value;	
if(select_area=="Select Nearest Area"){
supply_area();
}else{

$('#search-overlay__results').html('<div class="spinner-loader"></div>');
$('#0_client').empty();
window.setTimeout(function() {window.location.href = '#search_results';}, 100);
$('#search_results').html('Searching for Plans... Please wait!!');

$.getJSON(plancityData.root_url + '/wp-json/plancity/v1/search?age='+select_age+ '&select_area='+ select_area+ '&product_type='+ product_type+'&senior_citizen='+ senior_citizen+ '&select_director='+ select_director+ '&gender='+ gender
       			...
       			<div class='col-lg-4'>
        				<div class="properties_item">
        					<div align="center" class="pp_img">
        						<img class="img-fluid" src="${item.ClientLogo}">
        					</div>
							<h4 align="center">${item.ClientName}</h4>
							<h4 align="center">${item.ProvAreaName} Branch ${item.Distance ? `is +/- ${item.Distance}km` : ''}</h4>
        					<div class="pp_content">
								<h4 align="center">${item.plan_name}</h4>
        						<div align="center" class="tags">
									<a href="javascript:contacts(${item.ProviderId})"><i class="fa fa-check" aria-hidden="true"></i>Contacts</a>
									<a href="#"><i class="fa fa-check" aria-hidden="true"></i>Gallery</a>
        							<a href="javascript:services(${item.premium_id})"><i class="fa fa-check" aria-hidden="true"></i>Benefits</a>
									${item.additional_info ? `<a href="javascript:other(${item.premium_id})"><i class="fa fa-check" aria-hidden="true"></i>More Plan Info</a>` : ''}
         							<a href="javascript:limits(${item.premium_id})"><i class="fa fa-times" aria-hidden="true"></i>Limits</a>
        							<a href="javascript:probation(${item.premium_id})"><i class="fa fa-times" aria-hidden="true"></i>Probations</a>
 								</div>
								<h5>Premium: R ${item.Premium_Amount}pm</h5>  
									<h5>Joining Fee: ${item.Joining_Fee ? `R ${item.Joining_Fee}` : 'None'}</h5>
        						<div class="pp_footer">
        							
        							<a class="main_btn" href="#">Im Interested</a>
        						</div>
        					</div>
        				</div>
        			</div>
...
` : '<h4 align="center" fontcolor="red">Sorry No Results match your search criteria</h4>'}
...
$('#search_results').html('Our Top Search Results');
